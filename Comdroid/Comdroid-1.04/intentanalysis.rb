...
def getSendingActions(filedir)
	...
	printData("#{filedir}actionStats/sendingActions.txt", allSendingActions.uniq.sort)
end

def getReceivingActions(filedir)
	allReceivingActions=[]
	@component_names.each do |name|
		component = @components[name]
		component.actions.each do |action|
			allReceivingActions.push(action)
		end
	end

	printData("#{filedir}actionStats/receivingActions.txt", allReceivingActions.uniq.sort)
end
...
if $options[:measurements]
  IA.actionStats("#{IA.outfolder}")
  IA.getSendingActions("#{IA.outfolder}")
  IA.getReceivingActions("#{IA.outfolder}")
  IA.printData("#{IA.outfolder}actionStats/all_setActions.txt", IA.all_setActions.uniq.sort)

  IA.setActionNotFound.delete_if {|key, value| value==false }
  IA.printData("#{IA.outfolder}actionStats/setActionNotFound.txt", IA.setActionNotFound.keys.sort)

  IA.setDestinationNotFound.delete_if {|key, value| value==false }
  IA.printData("#{IA.outfolder}actionStats/setDestinationNotFound.txt", IA.setDestinationNotFound.keys.sort)
  IA.printData("#{IA.outfolder}actionStats/all_setDestinations.txt", IA.all_setDestinations.uniq.sort)
